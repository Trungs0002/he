...
func (h handler) processGetListRequest(c *gin.Context) (models.Scope, error) {
	...
	payload, ok := jwt.GetPayloadFromContext(ctx)
	if !ok {
		h.l.Warnf(ctx, "event.department.http.processGetRequest.GetPayloadFromContext: unauthorized")
		return models.Scope{}, pkgErrors.NewUnauthorizedHTTPError()
	}
	var query getQuery
	if err := c.ShouldBindQuery(&query); err != nil {
		h.l.Warnf(ctx, "event.department.http.processGetRequest.ShouldBindQuery: %v", err)
