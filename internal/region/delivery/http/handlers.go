...
func (h handler) listRegion(c *gin.Context) {
	...
	if err != nil {
		h.l.Warnf(ctx, "region.handler.listDepart.processGetRequest: %s", err)
		mapErr := h.mapError(err)
		response.Error(c, mapErr)
		return
	}
	...
	if err != nil {
		h.l.Warnf(ctx, "region.handler.listDepart.uc.GetListDepart: %s", err)
		mapErr := h.mapError(err)
		response.Error(c, mapErr)
		return
	}
...
func (h handler) get(c *gin.Context) {
	...
	if err != nil {
		h.l.Warnf(ctx, "region.handler.get.processGetRequest: %s", err)
		mapErr := h.mapError(err)
		response.Error(c, mapErr)
		return
	}
	...
	if err != nil {
		h.l.Warnf(ctx, "region.handler.get.uc.Get: %s", err)
		mapErr := h.mapError(err)
		response.Error(c, mapErr)
		return
	}
